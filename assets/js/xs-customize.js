      ...
      if ($("#sub-accordion-section-" + element).children('li').hasClass('xs_header_builder_switch')) {
        function customizer_value_dynamically_update(current, type, value) {
          ...
          let parents = current.parents('.control-section'), // control-section
              ...
              select_element = type === 'change' ? current.parents('.dm-option').next('.customize-control-select') : current.next().eq(0),// customize-control-select
              html_element = type === 'change' ? select_element.next('.dm-option') : select_element.next().eq(0); // xs_header_builder_html || xs_footer_builder_html
          ...
          if (type === 'change') {
            if (value === true) {
              ...
              current.parents('.dm-option').css('display', 'list-item'); // xs_footer_builder_switch || xs_header_builder_switch
            } else if (value === false) {
