  ...
  if ($(".xs_header_builder_select").length > 0) {
    function header_builder(current , type) {
        let id = type === 'change' ? current.val() : current.val(),
          ...
          current.parents('.fw-backend-option-input-type-multi').find(".xs_builder_edit_link").attr("href", admin_url);
    }
    $(".xs_header_builder_select").each(function () {
        header_builder($(this));
      $(".xs_header_builder_select").on("change", function (e) {
